...
epsilon_1_ratio = 0.5      # 一半预算给阈值选择，一半给位扰动
BF_error_rate = 0.01       # 目标误报率（决定 k）
num_repeat = 100
...
for epsilon_total in epsilon_total_list:
    ...
    for trial in range(num_repeat):
        ...
        # ---- 1. DP 阈值选择 (Exponential Mechanism, ε₁) ----
        utilities = []
        ...
        if not finite_mask.any():
            ...
            # 灵敏度 Δu = 1/|S|，但指数机制中灵敏度被 2*Δu 归一化，这里用相对值不影响概率分布
        ...
        if fn_count > 0:
            ...
            # 获取哈希函数数量 k
            k = 10  # pybloom_live 中哈希函数个数

            # 计算位扰动保留概率 p = e^{ε₂/k} / (e^{ε₂/k} + 1)
            if epsilon_2 > 0:
                exp_term = math.exp(epsilon_2 / k)
                p = exp_term / (exp_term + 1)
            else:
                p = 0.5  # 当 ε₂=0 时完全随机

            # 获取原始位数组并创建扰动后的位数组
            original_bits = bf.bitarray
            noisy_bits = bitarray(len(original_bits))
            for i in range(len(original_bits)):
                if rng.random() < p:
                    noisy_bits[i] = original_bits[i]
                else:
                    noisy_bits[i] = 1 - original_bits[i]

            # 用扰动后的位数组替换原 Bloom Filter 的位数组
            bf.bitarray = noisy_bits
    ...
    print(f"\n=== ε_total = {epsilon_total:.2f} (ε₁={epsilon_1:.3f}, ε₂={epsilon_2:.3f}) ===")
