...
labels = global_rng.choice([0, 1], size=num_samples)
...
for epsilon in epsilon_list:
    ...
    for _ in range(num_repeat):
        ...
        # 构建查询集合 Q
        Q_positive = rng_fixed.choice(S_target, min(num_query_each, len(S_target)), replace=False)
        Q_negative = rng_fixed.choice(N_notarget, min(num_query_each, len(N_notarget)), replace=False)
        Q = list(Q_positive) + list(Q_negative)
